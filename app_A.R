  ...
  dashboardBody(
    tabItems(
      tabItem(
        tabName = "desc_stats",
        fluidPage(
          titlePanel(title = div("Descriptive Statistics", style = "color: #333333; font-size: 40px; font-weight: bold; text-align: center; height: 120px")),
          mainPanel(
...
server <- function(input, output) {
  ...
  output$barplot_output <- renderPlotly({
    plots <- lapply(2:7, function(i) {
      plot_ly(data, x = ~data[, i], type = "histogram", color = I("#2596be")) %>%
        layout(showlegend = FALSE, title = colnames(data)[i], yaxis = list(title = "Frequency"))
    })
